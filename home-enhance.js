...
function enhanceLegacyHome(){
  ...
  if(!grid||!page){
    ...
    header?.classList.remove('legacy-hidden-header');
    return;
  }
  enhancing=true;
  try{
    ...
    cards.forEach((card,index)=>{
      ...
      if(img){
        const fallback=STORAGE_BASE+FALLBACKS[index%FALLBACKS.length];
        if(!img.getAttribute('src')||img.src.includes('picsum.photos')||img.src.includes('placehold.co'))img.src=fallback;
        img.onerror=()=>{img.onerror=null;img.src=fallback};
        const media=document.createElement('div');media.className='product-media';img.replaceWith(media);media.appendChild(img);
        media.insertAdjacentHTML('beforeend',`<span class="code-badge">#${String(card.dataset.id||'').slice(0,5).toUpperCase()}</span><span class="photo-badge">📷 3</span>`);
      }
      const body=[...card.children].find(x=>x!==card.querySelector('.product-media'))||card.lastElementChild;
      if(body){
        body.classList.add('legacy-card-body');
        const title=body.querySelector('h3');
        if(title&&!body.querySelector('.legacy-spec')){const spec=document.createElement('p');spec.className='legacy-spec';spec.textContent='លក់ដាច់គុណភាព 100%';title.after(spec)}
    ...
    if(search&&!search.dataset.bound){
      ...
      search.addEventListener('input',filter);document.getElementById('legacyClear')?.addEventListener('click',()=>{search.value='';filter();search.focus()});
    }
    top.querySelectorAll('.legacy-tabs button').forEach(btn=>{if(btn.dataset.bound)return;btn.dataset.bound='1';btn.onclick=()=>{top.querySelectorAll('.legacy-tabs button').forEach(x=>x.classList.remove('active'));btn.classList.add('active');const f=btn.dataset.filter;allCards.forEach(c=>c.style.display=f==='all'||c.textContent.toLowerCase().includes(f.toLowerCase())?'':'none');const visible=allCards.filter(c=>c.style.display!=='none').length;if(countEl)countEl.textContent=visible+' មុខ'}});
  }finally{enhancing=false;}
}

function scheduleEnhance(){
  clearTimeout(enhanceTimer);
  enhanceTimer=setTimeout(enhanceLegacyHome,120);
}
